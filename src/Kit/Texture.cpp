...
//std::map<std::string, kit::Texture::Ptr> kit::Texture::m_cache = std::map<std::string, kit::Texture::Ptr>();

std::map<std::string, kit::Texture::Ptr> kit::Texture::m_cachedTextures = std::map<std::string, kit::Texture::Ptr>();
std::map<std::string, kit::Texture::Ptr> kit::Texture::m_cachedCubemaps = std::map<std::string, kit::Texture::Ptr>();

kit::Texture::Texture(Type t){
  
  KIT_GL(glCreateTextures(t, 1, &this->m_glHandle));

  //TODO: this->m_anisotropicLevel = 0.0;
...
kit::Texture::Texture(GLuint handle){
...
kit::Texture::Ptr kit::Texture::reference(GLuint handle)
{
  kit::Texture::Ptr returner = std::make_shared<kit::Texture>(handle);
  return returner;
}
...
kit::Texture::Ptr kit::Texture::createRaw(std::string filename, glm::uvec2 resolution)
{
  std::cout << "Loading raw texture from file " << filename.c_str() << std::endl;
  kit::Texture::Ptr returner = std::make_shared<kit::Texture>(Texture2D);
  returner->m_internalFormat    = R16F;
  returner->m_resolution = glm::uvec3(resolution, 0);

  // Try to load data from file
  std::vector<uint16_t>  bufferData;

  std::ifstream f(filename, std::ios_base::binary);
  if(!f)
  {
    KIT_ERR("createRaw failed: could not open file.");
    return returner;
  }
  
  uint16_t reader;
  for(uint32_t i = 0; i < resolution.x*resolution.y;i++)
  {
    
    f.read((char*)&reader, 2);
    //reader.asUint32 = kit::networkDecode32(reader.asUint32);
    bufferData.push_back(reader);
  }
  f.close();

  // Specify storage and upload data to GPU
  KIT_GL(glTextureStorage2D(returner->m_glHandle, 1, returner->m_internalFormat, returner->m_resolution.x, returner->m_resolution.y));
  KIT_GL(glTextureSubImage2D(returner->m_glHandle, 0, 0, 0, returner->m_resolution.x,  returner->m_resolution.y, GL_RED, GL_UNSIGNED_SHORT, &bufferData[0]));


  
  // Set parameters
  returner->setEdgeSamplingMode(kit::Texture::ClampToEdge);
  returner->setMinFilteringMode(kit::Texture::Linear);
  returner->setMagFilteringMode(kit::Texture::Linear);

  returner->setAnisotropicLevel(1.0f);
...
kit::Texture::Ptr kit::Texture::createCubemap(glm::uvec2 faceresolution, kit::Texture::InternalFormat format, kit::Texture::FilteringMode minfilter, kit::Texture::FilteringMode magfilter)
{
  
  kit::Texture::Ptr returner = std::make_shared<kit::Texture>(Cubemap);
  returner->m_internalFormat    = format;
  returner->m_resolution        = glm::uvec3(faceresolution, 0);
...
/*
kit::Texture::Ptr kit::Texture::createCubemapFromFiles(kit::Texture::FaceList filenames, kit::Texture::InternalFormat format, kit::Texture::FilteringMode minfilter, kit::Texture::FilteringMode magfilter)
{
  kit::Texture::Ptr returner = std::make_shared<kit::Texture>(Cubemap);
  returner->m_internalFormat    = format;
  ...
  // Set resolution
  returner->m_resolution        = kit::Vec3U32(x, y, 0);
  
  // Specify storage and upload data to GPU
  KIT_GL(glTextureStorage2D(returner->m_glHandle, returner->calculateMipLevels(), returner->m_internalFormat, returner->m_resolution.x, returner->m_resolution.y));
  KIT_GL(glTextureSubImage2D(this->m_glHandle, cubeface, 0, 0, x, y, GL_RGBA, GL_UNSIGNED_BYTE, bufferdata));
...
kit::Texture::Ptr kit::Texture::loadContribMap(std::string name)
{
  kit::Texture::Ptr returner = kit::Texture::create2DFromFile(std::string("./data/terrains/") + name + std::string("/contribmap.tga"), RGBA8, ClampToEdge,kit::Texture::LinearMipmapLinear, kit::Texture::Linear);
  returner->setAnisotropicLevel(8.0f);
  return returner;
}
...
kit::Texture::Ptr kit::Texture::loadIrradianceMap(std::string name)
{
  return nullptr;
}

kit::Texture::Ptr kit::Texture::loadRadianceMap(std::string name)
{
  return nullptr;
}

kit::Texture::Ptr kit::Texture::loadSkybox(std::string name)
{
  return nullptr;
}

bool kit::Texture::updatePixelsFromFile(std::string filename, Face f)
{
  ...
  // Upload data to GPU
  if(this->m_type == kit::Texture::Texture2D)
  {
    if(x != this->m_resolution.x || y != this->m_resolution.y)
    {
      KIT_ERR("Tried to update texture from file of different format");
      return false;
    }
    KIT_GL(glTextureSubImage2D(this->m_glHandle, 0, 0, 0, x, y, GL_RGBA, GL_UNSIGNED_BYTE, bufferdata));
  }
  else if(this->m_type == kit::Texture::Cubemap)
  {
    KIT_GL(glTextureSubImage3D(this->m_glHandle, 0, 0, 0, (uint32_t)f, x, y, 0, GL_RGBA, GL_UNSIGNED_BYTE, bufferdata));
  }
...
bool kit::Texture::updatePixelsFromFiles(kit::Texture::FaceList filenames)
{
  for(uint32_t i = FaceBegin; i != FaceEnd; i++)
  {
    if(!this->updatePixelsFromFile(filenames[i]))
    {
      return false;
    }
  }

...
uniform sampler2D uniform_bloomDirtTexture;
uniform float uniform_bloomDirtMultiplier = 3.0;
...
void main( ) {
    ...
    /*vec3 bloom2 = vec3(texture(uniform_bloom2Texture, in_uv).r)   * 0.000229;
    vec3 bloom4 = vec3(texture(uniform_bloom4Texture, in_uv).r)   * 0.005977;
    vec3 bloom8 = vec3(texture(uniform_bloom8Texture, in_uv).r)   * 0.060598;
    vec3 bloom16 = vec3(texture(uniform_bloom16Texture, in_uv).r) * 0.241732;
    vec3 bloom32 = vec3(texture(uniform_bloom32Texture, in_uv).r) * 0.382928;*/
    ...
    vec3 bloom = (bloom2 + bloom4 + bloom8 + bloom16 + bloom32);

    vec3 bloomDirt = texture(uniform_bloomDirtTexture, in_uv).rgb * bloom * uniform_bloomDirtMultiplier;
    
    vec3 in_color = texture(uniform_sourceTexture, in_uv).rgb * uniform_exposure;
    out_color.a = 1.0;
    out_color.rgb = (Uncharted2Tonemap(in_color+bloom+bloomDirt) / Uncharted2Tonemap(vec3(uniform_whitepoint)));
}